# iNutri — Copy da Landing Page (v1.2)

> **Preços:** `PLAN_CATALOG` (`packages/shared-types/src/v1/billing.ts`) é a fonte de
> verdade. Atualizado em 2026-08-28 para Essencial R$39/R$390 e Pro R$79/R$790.
> A âncora competitiva "~R$90+ nos líderes" NÃO foi verificada — ver ressalva na
> análise de margem antes de reutilizar esse número.

**Status:** vigente na LP  
**Data:** 2026-08-12 · **Rev.:** 1.2 (fluidez + tom de voz do manifesto)  
**CTA principal (único, em toda a página):** **Começar 7 dias grátis**  
**ICP:** nutricionista solo (consultório / atendimento online) no Brasil  
**Tom:** profissional, humano, sereno — manifesto em `docs/brand/manifesto.md`

---

## Changelog v1.1 → v1.2 (tom de voz)

Reescrita jornalística guiada pelo manifesto da marca. Mantém a tese de conversão (dor × solução, frases de efeito, um CTA). Suaviza o léxico e os tempos verbais.

| # | Antes (v1.1) | Depois (v1.2) |
|---|---|---|
| 1 | “caçar conversa / print” | “recuperar conversas” / “procurar conversas e prints” |
| 2 | “sem bagunça” / “zero gambiarra” | “em um só fluxo” / “sem improvisar” |
| 3 | “come sua noite” / “paciente some” | “ocupa a noite” / “o paciente se afasta” |
| 4 | “morrer no e-mail” / “roubar a noite” | “ficar esquecido no e-mail” / “ocupar a noite” |
| 5 | “tour turístico” / “sem drama” | “passeio pelas telas” / “sem complicação” |
| 6 | “coleciona software” | “acumula ferramentas” |
| 7 | Imperativo + gíria empilhada | Presente do indicativo na descrição; imperativo só em CTA e passos |

Fonte de tom: `docs/brand/manifesto.md`.

---

## Changelog v1 → v1.1 (o que a revisão melhorou)

| # | Achado | Ajuste |
|---|---|---|
| 1 | CTAs diferentes (“Criar conta”, “Quero testar…”) diluem o hábito de clicar | **Um CTA primário** em todo o funil: *Começar 7 dias grátis* |
| 2 | Dor citava “sem lembrete” e o produto pode não ter push no go-live | Dor/solução **sem prometer lembrete/push** (fast-follow) |
| 3 | “Cota mensal de IA” sem número gerava ansiedade | Essencial: **até ~30 gerações/mês** (alinhado ao doc de pricing); Pro: **até 200 gerações/mês** |
| 4 | Prova social fraca no lançamento | Bloco honesto **“Em lançamento”** + o que o trial entrega em 7 dias |
| 5 | Faltava reduzir fricção pós-clique | Seção curta **“O que acontece depois do clique”** |
| 6 | Preço sem âncora cotidiana | **Menos de R$2/dia** (Essencial) + economia no anual em R$ |
| 7 | “Assina com a sua expertise” soava a assinatura digital | **“Você valida e personaliza”** |
| 8 | Página longa demais se implementada 1:1 | Marcação **[essencial]** / **[opcional]** por seção |
| 9 | Subheadline do hero um pouco genérica | Mais concreta: **noite/Word/Excel** + resultado em 1 frase |
| 10 | Silhueta com “wow” informal demais | Tom mais profissional, mantendo desejo |
| 11 | Faltava risk reversal forte perto do preço | Linha de **risco zero** sob pricing e no CTA final |
| 12 | Features de suporte misturavam “pode ter / não ter” | Lista só do que o posicionamento de go-live assume; resto fora |

---

## Decisões estratégicas (ler antes da copy)

### 1) Trial de 7 dias — sim, é a melhor escolha agora

| Opção | Prós | Contras |
|---|---|---|
| **7 dias** ✅ | Urgência; uso real na 1ª semana; CAC menor; alinhado ao Asaas | Menos “folga” que Dietbox (30 dias) |
| 14–30 dias | Mais tempo para explorar | Atrasa decisão; mais “turista”; pior unit economics |

**Como vender o 7 dias:** “tempo para **usar na rotina** (plano real + app +, no Pro, Silhueta)” — não tour de features.

**Onboarding sugerido (produto, não LP):**  
Dia 1 → 1 paciente + 1 plano com IA · Dia 2–3 → app do paciente · Dia 4+ → agenda / Silhueta (Pro).

**Billing:** cartão no cadastro + 1ª cobrança em D+7 (converte melhor). Pix no anual.  
Microcopy: **Cancele quando quiser. Sem multa. Sem fidelidade.**

### 2) Prova social — **não forje depoimentos**

Depoimento inventado (nome, CRN, foto) = risco legal + quebra de confiança. **Não usar.**

**No lançamento (honesto e conversível):**

1. Prova de produto — screenshots / GIF do fluxo real  
2. Prova de processo — solo, LGPD, consentimento Silhueta  
3. Risco zero — trial + cancelamento  
4. Âncora de preço — a partir de R$39 vs líderes ~R$90+  
5. Badge **Em lançamento** (verdade)  
6. Depois: 3–5 depoimentos reais com autorização por escrito  

### 3) Âncora de mensagem — 1 dor + 3 pilares

**Dor principal:** tempo e cansaço operacional (plano + admin + paciente some entre consultas).

| Ordem | Pilar | Função |
|---|---|---|
| 1º | **IA de planos** | Hook de tempo → clique no CTA |
| 2º | **App do paciente** | Adesão / valor contínuo |
| 3º | **Silhueta** | Diferencial premium → plano **Pro** |

### 4) Posicionamento vs. concorrentes

| Player | Empurram | Abertura iNutri |
|---|---|---|
| **Dietbox** | Ecossistema + app (~R$92/mês) | Enxuto, solo, **R$39** no Essencial |
| **WebDiet** | Clara IA + Body3D em planos altos | Silhueta no Pro + preço competitivo |
| **Nutrium** | Global / flexível | BR-first, fluxo simples |
| **Legados** | Desktop / tabelas | Moderno + IA + app |

**Promessa em uma frase:**  
> Software moderno para o nutricionista solo: plano com IA, app do paciente e avaliação por foto — sem pagar preço de clínica grande.

**Nunca prometer:**

- IA substitui o nutricionista  
- Silhueta = diagnóstico / = bioimpedância ou DEXA  
- Usuários, CRNs ou depoimentos inventados  
- Lembrete/push, anamnese completa, etc., se não estiverem no ar  
- Features ainda em construção no dia do go-live  

### 5) Comprimento da página

| Seção | Prioridade |
|---|---|
| Hero, dor, 3 passos, 3 pilares, pricing, FAQ curto, CTA final | **[essencial]** |
| Comparativo, para quem é/não é, bloco “em lançamento” detalhado | **[recomendado]** |
| Features de suporte longas, tabela larga no mobile | **[opcional]** — encurtar no mobile |

---

# COPY COMPLETA DA LANDING (v1.2)

> Legenda: `[CTA]` · `[nota UI]` · `[essencial|recomendado|opcional]`

---

## 0) Meta / SEO **[essencial]**

**Title (≤60):**  
iNutri | Planos com IA e app do paciente para o nutri solo

**Description (≤155):**  
Menos tempo montando plano. Mais tempo no atendimento. IA para planos, app grátis para o paciente, agenda e Silhueta. 7 dias grátis. Cancele quando quiser.

**H1** = headline do hero (única H1 da página).

---

## 1) Header / barra superior **[essencial]**

**Nav:** Logo · Como funciona · Preços · FAQ · **Entrar**  
**CTA fixo (desktop + sticky no mobile):** Começar 7 dias grátis  

**Barra opcional (announce):**  
7 dias grátis · Sem fidelidade · Paciente usa o app sem pagar  

---

## 2) Hero **[essencial]**

### Eyebrow
Feito para nutricionistas que atendem sozinhos

### Headline
**Menos tempo no plano.  
Mais tempo no consultório.**

### Subheadline
Monte planos com **IA**, compartilhe no **app do paciente** e acompanhe o atendimento em um só fluxo.

### CTAs
**[Primário]** Começar 7 dias grátis  
**[Secundário]** Ver como funciona ↓ → âncora `#como-funciona`

### Microcopy sob o botão
7 dias grátis · Cancele quando quiser · A IA acelera — **você decide** o plano

### Proof strip (sem depoimento falso)
- A partir de **R$39/mês** (menos de **R$2/dia**)  
- App do paciente **incluso e gratuito**  
- Planos com **IA + banco de alimentos**  
- **Silhueta** (estimativa por foto) no **Pro**

`[nota UI: 4 itens em linha no desktop; grade 2×2 no mobile. Ícones discretos. Hero com mockup web + celular se possível.]`

### Faixa “Em lançamento” (1 linha, sob o hero ou no proof strip)
**Em lançamento** — o essencial do consultório: plano, paciente e rotina.

---

## 3) Seção de dor **[essencial]**

### Título
**Depois da consulta, o trabalho continua. Entre uma e outra, o paciente se afasta.**

### Intro
Você se formou para cuidar de pessoas — não para passar as noites em planilhas, reescrever cardápios e recuperar conversas no WhatsApp.

### Custo da dor (1 linha, em destaque)
**Uma consulta bem feita não deveria se transformar em mais uma hora de trabalho invisível em casa.**

### Cards (3 colunas)

**1. O plano que ocupa a noite**  
Montar o cardápio, ajustar os macronutrientes, gerar o PDF e reenviar. O atendimento termina; o trabalho, não.

**2. O paciente que se afasta**  
O plano permanece em um PDF no e-mail. Fora da consulta, o contexto se perde — e a adesão enfraquece.

**3. Software caro demais para o solo**  
Você paga o preço de uma clínica grande por telas e módulos que o dia a dia quase não utiliza.

### Ponte
**O iNutri devolve tempo e continuidade ao consultório solo.**  
**[CTA]** Começar 7 dias grátis

---

## 4) Como funciona `#como-funciona` **[essencial]**

### Título
**Do cadastro ao plano no celular do paciente — em minutos, não em horas.**

### Passos

**1. Cadastre o paciente**  
Dados, metas e preferências ficam no mesmo lugar. Sem planilha à parte.

**2. Gere o plano com IA e valide com o seu critério**  
A IA usa o perfil daquele paciente para montar um rascunho sólido. Você edita, personaliza e **decide**.  
*A IA sugere. Você conduz.*

**3. Disponibilize no app do paciente**  
O paciente vê o plano no celular. Você acompanha a evolução e o histórico sem procurar conversas e prints no WhatsApp.

### Fecho
**[CTA]** Começar 7 dias grátis

---

## 5) Três pilares **[essencial]**

### Título
**O essencial do consultório solo, em um só fluxo.**

### Intro
Três pilares. Um caminho contínuo. Sem improvisar entre planilha, PDF e várias abas abertas.

---

### Pilar 1 — Planos com IA  
**Do rascunho ao PDF em uma fração do tempo.**

- Gere um plano alinhado a metas e restrições do paciente  
- Ajuste refeições, quantidades e preferências no editor  
- Use banco de alimentos com informações nutricionais  
- Exporte PDF profissional quando precisar  

**Em uma frase:** energia no raciocínio clínico — não na digitação.

**Badge:** Essencial (até ~**30 gerações de IA/mês**) · Pro (**até 200 gerações de IA/mês**)

---

### Pilar 2 — App do paciente (incluso e grátis)  
**O plano deixa de ficar esquecido no e-mail.**

- O paciente acessa o plano no celular  
- Menos pedidos para reenviar o PDF  
- Mais presença e clareza entre as consultas  
- Uma experiência que eleva a percepção do seu atendimento  

**Em uma frase:** o consultório permanece presente depois que a consulta termina.

**Badge:** Incluso em todos os planos · **paciente não paga**

---

### Pilar 3 — Silhueta  
**Estimativa de composição corporal por foto — ideal para teleconsulta e atendimento diferenciado.**

- Fluxo com **consentimento** claro  
- Fotos usadas na estimativa e **não armazenadas** no iNutri  
- Relatório para **tendência do mesmo paciente** (Silhueta × Silhueta)  
- Transparência: **estimativa, não diagnóstico**  

**Em uma frase:** engaja o paciente e enriquece o acompanhamento — com ética e clareza.

**Badge:** Exclusivo do **Pro** · *não substitui bioimpedância ou DEXA*

`[nota UI: disclaimer legível sob o card; evitar visual “médico milagroso”]`

---

### Também no dia a dia **[recomendado — chips curtos]**

- Agenda de consultas  
- Bioimpedância e evolução no prontuário  
- Metas nutricionais e cálculos de apoio  
- Contabilidade simples do consultório *(Pro)*  

`[nota UI: não listar push, anamnese completa, marketing, etc., se não estiverem no ar]`

---

## 6) O que você faz nos 7 dias grátis **[recomendado]**

### Título
**Um teste de rotina — não um passeio pelas telas.**

| Dia | Faça isto |
|---|---|
| **1** | Crie conta, cadastre 1 paciente, gere 1 plano com IA e ajuste |
| **2–3** | Libere o app para o paciente (ou use em modo demonstração) |
| **4–7** | Use agenda / evolução; no **Pro**, rode uma Silhueta com consentimento |

Se ao fim da semana o iNutri **não devolver tempo**, cancele. Sem complicação.

**[CTA]** Começar 7 dias grátis

---

## 7) Para quem é / não é **[recomendado]**

### Título
**Feito para quem atende. Não para quem acumula ferramentas.**

### Para quem é
- Nutricionista **solo** ou com rotina enxuta  
- Quem monta **vários planos por semana** e sente o peso operacional  
- Quem atende **presencial, online ou híbrido** e quer o paciente no app  
- Quem quer **IA como acelerador**, não como piloto automático  

### Para quem não é
- Clínicas que precisam de ERP hospitalar completo  
- Quem só quer tabela de alimentos offline, sem acompanhamento  
- Quem espera que a IA substitua conduta e julgamento clínico  

---

## 8) Comparativo de valor **[recomendado]**

### Título
**Menos software. Mais resultado por real investido.**

| | Planilhas / PDF solto | Softwares “tudo e mais um pouco” | **iNutri** |
|---|---|---|---|
| Tempo no plano | Alto | Médio (se dominar a ferramenta) | **Baixo — IA + editor** |
| App do paciente | Não | Sim (em geral) | **Sim · grátis pro paciente** |
| Avaliação por foto | Não | Em planos caros | **Silhueta no Pro** |
| Preço solo | “Grátis” que custa noites | ~R$90+/mês nos líderes | **A partir de R$39/mês** |
| Curva de aprendizado | Caótica | Pesada | **Enxuta, do atendimento** |

### Fecho
Você não precisa do software mais antigo do mercado.  
Precisa do que **devolve o seu tempo**.

**[CTA]** Começar 7 dias grátis

`[nota UI: no mobile, preferir 3 cards empilhados em vez de tabela larga]`

---

## 9) Pricing `#precos` **[essencial]**

### Título
**Preço de nutricionista solo. Não de rede de clínicas.**

### Sub
7 dias grátis em qualquer plano. **Cancele quando quiser — sem multa e sem fidelidade.**

### Toggle
**Mensal** | **Anual** (2 meses grátis)

---

### Card — Essencial  
**R$39/mês**  
*ou **R$390/ano** (~R$32,50/mês) · **economize R$78/ano***  
*Menos de **R$2 por dia**.*

**Para organizar o consultório e acelerar planos.**

Inclui:
- Pacientes ilimitados  
- Editor de planos + banco de alimentos  
- IA de planos — **até ~30 gerações/mês**  
- App do paciente (paciente não paga)  
- Agenda  
- Bioimpedância e evolução  
- Exportação em PDF  

Não inclui: Silhueta · contabilidade · cota ampliada de IA  

**[CTA]** Começar 7 dias grátis  
`[nota UI: pode ir para signup?plan=essencial]`

---

### Card — Pro · **Recomendado**  
**R$79/mês**  
*ou **R$790/ano** (~R$66/mês) · **economize R$158/ano***

**Para velocidade máxima e atendimento diferenciado.**

Tudo do Essencial, mais:
- **IA de planos: até 200 gerações/mês**  
- **Silhueta** (estimativa por foto)  
- **Contabilidade** do consultório  

**[CTA em destaque]** Começar 7 dias grátis  
`[nota UI: signup?plan=pro · visualmente o card “escolhido”]`

---

### Risk reversal (sob os cards)
**Nos sete dias, você percorre o fluxo completo.**  
Não gostou? Cancele antes do fim do período de teste e **não será cobrado**.  
No Pro, Silhueta e a cota ampliada de IA entram no plano — **sem custo extra escondido**.

### Âncora de mercado (discreta)
*Líderes do mercado costumam ficar na casa dos R$90+/mês no plano completo — nem sempre com a simplicidade que o consultório solo precisa.*

---

## 10) O que acontece depois do clique **[recomendado]**

### Título
**Começar é simples. Voltar à planilha, nem tanto.**

1. Crie sua conta em poucos minutos  
2. Cadastre um paciente (pode ser de teste)  
3. Gere um plano com IA e veja o app do paciente  

Sem reunião comercial. Sem agendar uma demonstração para ver o básico.

**[CTA]** Começar 7 dias grátis

---

## 11) FAQ **[essencial — 6 a 8 perguntas no ar; resto colapsado]**

### Título
**Perguntas que nutricionistas fazem antes de testar**

**A IA vai padronizar meus pacientes?**  
Não. A IA observa os **dados únicos de cada paciente** — metas, restrições, preferências e o que você registra no acompanhamento — para montar um **ponto de partida personalizado**, não um cardápio genérico. Você edita, corta, troca e finaliza. A conduta continua sendo sua.

**7 dias são suficientes?**  
Sim, se for um teste de rotina: 1–2 pacientes, um plano com IA, app liberado e (no Pro) uma Silhueta. Não precisa “conhecer cada botão”.

**Preciso de cartão para o trial?**  
`[AJUSTAR AO ASAAS FINAL]`  
*Se cartão obrigatório (recomendado p/ conversão):*  
Sim, para ativar o trial. A **1ª cobrança só após os 7 dias**. Cancele antes e não é cobrado.  
*Se cartão opcional:*  
Não no início. Você escolhe o plano ao fim do trial (cartão ou Pix no anual).

**O paciente paga alguma coisa?**  
Não. O app está incluso. Quem assina é o nutricionista.

**Silhueta substitui bioimpedância?**  
Não. É **estimativa por foto**, com consentimento, para tendência e engajamento — **não é diagnóstico** e não deve ser tratada como igual a outros métodos.

**E a LGPD / fotos do paciente?**  
A Silhueta exige **consentimento**. As fotos são usadas na estimativa e **não ficam armazenadas** no iNutri. Há política de privacidade e controles de conta.

**Consigo cancelar fácil?**  
Sim. Sem fidelidade e sem “ligue para o comercial”.

**Já uso outro software. Vale migrar?**  
Se você paga caro por telas que não usa — ou ainda vive entre PDF e WhatsApp — o trial mostra na prática se o iNutri devolve tempo. Sem discurso: com uso real.

**Funciona para atendimento online?**  
Sim. Plano no app e Silhueta por foto também servem quem atende à distância.

---

## 12) CTA final **[essencial]**

### Headline
**Sua próxima consulta não precisa ocupar a sua noite.**

### Sub
Teste por sete dias. Monte um plano com IA, coloque o paciente no app e avalie a diferença — sem compromisso.

### CTA
**[Primário grande]** Começar 7 dias grátis  

### Microcopy
Cancele quando quiser · Paciente grátis · Você no comando do plano  

### Confiança (sem fake)
Feito no Brasil, para o nutricionista brasileiro.  
Privacidade e transparência no uso de IA.  
**Em lançamento** — pensado para o dia a dia do consultório solo.

---

## 13) Footer **[essencial]**

- Como funciona · Preços · FAQ · Suporte · Privacidade · Termos  
- © iNutri  
- *iNutri é ferramenta de apoio ao nutricionista. Não substitui julgamento clínico, diagnóstico médico ou exames.*  
- *Silhueta é estimativa por foto e não substitui métodos de avaliação clínica consolidados.*

---

# VARIANTES DE HEADLINE (A/B futuro)

1. **Menos tempo no plano. Mais tempo no consultório.** ← principal  
2. Pare de montar plano na madrugada.  
3. Plano com IA. Paciente no app. Consultório sob controle.  
4. Seu paciente merece mais que um PDF no e-mail.  
5. O software do nutri solo — sem preço de clínica grande.

**Subheadline A/B (preço na dobra):**  
A partir de R$39/mês — IA de planos e app do paciente incluso. 7 dias grátis.

---

# MICROCOPY DOS BOTÕES (banco v1.2)

Mesmo destino (`/signup`), **labels variam por seção** para não ficar monótono — objetivo sempre = trial 7 dias.

| Contexto | Texto |
|---|---|
| Header | Testar grátis |
| Hero | **Começar 7 dias grátis** |
| Após dor | Quero recuperar meu tempo |
| Como funciona | Criar minha conta grátis |
| Trial 7 dias | Iniciar meus 7 dias |
| Comparativo | Experimentar o iNutri grátis |
| Pricing Essencial | Começar no Essencial |
| Pricing Pro | Começar no Pro |
| Após o clique | Criar conta em minutos |
| FAQ | Testar sem compromisso |
| CTA final | Garantir meus 7 dias grátis |
| Sticky mobile | Testar 7 dias grátis |
| Secundário | Ver como funciona |
| Header (já tem conta) | Entrar |

**Evitar no hero:** Saiba mais · Enviar · Cadastrar · Assinar  

**Nota:** o destino e a promessa (trial grátis) permanecem iguais; só a frase muda.

---

# CHECKLIST ANTES DE PUBLICAR

1. Features no ar = features na copy (TACO/macros, GET/TMB, cota de IA, trial Asaas).  
2. Screenshots/GIF reais no hero e nos pilares.  
3. Decisão **cartão no trial** → travar a FAQ correspondente.  
4. Links Privacidade + Termos.  
5. Não publicar depoimento sem autorização e pessoa real.  
6. Confirmar números contra `PLAN_CATALOG`: R$39 / R$79 / R$390 / R$790 / 30 gerações Essencial / 200 Pro.  
7. Mobile: tabela do comparativo vira cards; CTA sticky.  
8. Tracking: clique do CTA, scroll até preços, início do signup.

---

# RESUMO DA TESE DE CONVERSÃO (v1.1)

1. **Espelhar a dor** (tempo + paciente sumido + software caro)  
2. **Prometer tempo de volta** (headline curta e escaneável)  
3. **Mostrar o sistema** (3 passos + 3 pilares)  
4. **Reduzir risco** (7 dias + cancelar fácil + “teste de rotina”)  
5. **Âncora de preço** (R$39 · &lt;R$2/dia · vs ~R$90+)  
6. **Upsell natural** (Pro = 200 gerações/mês + Silhueta + contabilidade)  
7. **Um CTA** em toda a página  
8. **Honestidade clínica** (IA não substitui; Silhueta não diagnostica)  
9. **Sem promessas de features futuras** na LP pública  

---

*Fim da v1.2. Tom alinhado a `docs/brand/manifesto.md`. Aplicado na landing Next.js.*
