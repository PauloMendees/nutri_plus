...
describe('SignupForm', () => {
  ...
  it('signs up with name metadata + callback redirect, then routes to verify-email', async () => {
    ...
    // Vai canonicalizado nos metadados; o backend grava no perfil ao sincronizar.
    expect(arg.options.data.whatsapp).toBe('5511999998888');
  ...
  it('requires WhatsApp', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/confirmar senha/i), 'supersecret');
    await userEvent.click(screen.getByRole('button', { name: /criar conta/i }));
    expect(await screen.findByText(/informe seu whatsapp/i)).toBeInTheDocument();
    expect(signUp).not.toHaveBeenCalled();
  });
