...
function lockReasonText(
  ...
  if (lockReason === 'demo') return 'Cadastre o paciente de demonstração primeiro.';
...
export function HubView({ role }: { role: UserRole | null }) {
  ...
  function play(chapterId: string | null, replay = false) {
    ...
    if ((status === 'completed' || status === 'skipped') && !replay && !recoveringCadastro) return;
      ...
      <div className="space-y-5">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Tutoriais guiados pelos módulos do iNutri. Cada capítulo destaca a tela certa: clique no
            que está iluminado ou em Próximo. Você pode pular, sair e rever quando quiser.
          </p>
        </div>
          ...
          <CardContent>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {PATIENTS_TOUR.chapters.map((chapter) => {
                ...
                const reason = view.status === 'locked' ? lockReasonText(view.lockReason, entitlements) : null;
                return (
                  <Card key={chapter.id} size="sm" data-chapter={chapter.id} className="gap-2 py-3">
